...
final pokemonSpeciesProvider = FutureProvider.family<PokemonSpecies, int>((ref, id) async {
  final apiService = ref.read(apiServiceProvider);
  final response = await apiService.get("https://pokeapi.co/api/v2/pokemon-species/$id");
  return PokemonSpecies.fromJson(response.data);
});

final evolutionChainProvider = FutureProvider.family<PokemonEvolutionModel, int>((ref, id) async {
  ...
  final apiService = ref.read(apiServiceProvider);
  final response = await apiService.get("https://pokeapi.co/api/v2/evolution-chain/$evolutionId");
